...
- (NSArray *)executeSql:(NSString *)sql withParameters:(NSArray *)parameters withClassForRow:(Class )rowClass
{
    ...
    {
        NSLog(@"executeSql:%@ withParameters:%@ withClassForRow:%@ ERROR:\n%@", sql, parameters, rowClass, exception);
        
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"" message:NSLocalizedString(@"Error of the database", @"") delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
        [alertView show];
    }
    @finally
    {
...
- (NSArray *)columnsForTableName:(NSString *)tableName
{
    ...
    NSMutableArray *columns = [NSMutableArray arrayWithArray:[results valueForKey:@"name"]];
 
    NSPredicate *idTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", kMojoDatabaseForeignKeyFieldFormat];

    for (NSDictionary *columnInfo in results)
    {
        NSString *key = [columnInfo objectForKey:@"name"];
        if ([idTest evaluateWithObject:key])
            [columns removeObject:key];
    }
                    ...
                    columnNames:(NSArray *)columnNames
{
    ...
    for (int i = 0; i < columnCount; i++) {
        ...
        if (value != nil)
        {
            NSString *key = [columnNames objectAtIndex:i];

            NSPredicate *idTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", kMojoDatabaseForeignKeyFieldFormat];
            if (![idTest evaluateWithObject:key])
            {
                NSPredicate *dateTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", kMojoDatabaseDateFieldFormat];
                
                if ([key isEqualToString:@"createdAt"] || [key isEqualToString:@"updatedAt"] || [dateTest evaluateWithObject:key])
                {
                    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                    dateFormatter.dateFormat = kMojoDatabaseDateFormat;
                    value = [dateFormatter dateFromString:value];
                }

                if ([[columnTypes objectAtIndex:i] intValue] == SQLITE_TEXT)
                {
                    NSPredicate *booleanTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", kMojoDatabaseBooleanFieldFormat];

                    if ([booleanTest evaluateWithObject:key])
...
- (id)valueFromStatement:(sqlite3_stmt *)statement column:(int)column queryInfo:(NSDictionary *)queryInfo columnTypes:(NSArray *)columnTypes
{
    ...
    */
    ...
    else if (columnType == SQLITE_TEXT)
    {
        ...
        return text != nil ? [NSString stringWithUTF8String:text] : nil;
    }
...
- (void)bindArguments:(NSArray *)arguments toStatement:(sqlite3_stmt *)statement queryInfo:(NSDictionary *)queryInfo
{
    ...
    for (int i=1; i<=expectedArguments; i++)
    {
        ...
        else if([argument isKindOfClass:[NSDate class]])
        {
            NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
            dateFormatter.dateFormat = kMojoDatabaseDateFormat;
            NSString *dateString = [dateFormatter stringFromDate:argument];
            sqlite3_bind_text(statement, i, [dateString UTF8String], -1, SQLITE_TRANSIENT);
